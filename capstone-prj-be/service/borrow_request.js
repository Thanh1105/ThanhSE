...
const validate_admin_setting = async (interest_rate, expected_money) => {
    ...
    if (expected_money < admin_setting.min_lending_expected_money) {
        throw new Error("expected money must be bigger than " + admin_setting.min_lending_expected_money)
    }
}
...
const handle_rollback_money = async (borrow_request, from_payment=false) => {
    ...
    try {
        ...
        for(const account_information of account_informations ){
            for(const invest_request of borrow_request.investment_requests){
                if(invest_request.user_id == account_information.user_id){
                    ...
                    if (from_payment){
                        recieve_money = Math.ceil(Number(invest_request.amount) * (1 + Number(borrow_request.interest_rate)*(1 - Number(invest_request.discount))))
                        subtract_money += Math.ceil(Number(invest_request.amount) * (1 + Number(borrow_request.interest_rate)))
        ...
        if ( from_payment){
            ...
            borrower_account_information.balance = Number(borrower_account_information.balance) - subtract_money
            await borrower_account_information.save({ transaction: t })
            const borrow_request_content = "your borrow_request has been payment successful"
        ...
        const a = from_payment ? 1.3 :1
        borrower_user.limit_money = Math.ceil(borrow_request.expected_money * a) + Number(borrower_user.limit_money)
...
module.exports = {
    ...
    async list(req) {
        ...
        const limit_bottom = Number(_.get(req, "query.limit_bottom", 0))
        const limit_top =  Number(_.get(req, "query.limit_top", 0))
