...
describe('Notification e2e test', () => {
  ...
  describe('new Notification page', () => {
    ...
    it('should create an instance of Notification', () => {
      cy.get(`[data-cy="typeNoti"]`).type('99754').should('have.value', '99754');

      cy.get(`[data-cy="typeAccount"]`).type('31503').should('have.value', '31503');

      cy.get(`[data-cy="message"]`).type('deposit Kansas open-source').should('have.value', 'deposit Kansas open-source');

      cy.get(`[data-cy="hasRead"]`).should('not.be.checked');
      cy.get(`[data-cy="hasRead"]`).click().should('be.checked');
